...
// draw a 8x16 px char without background
...
// draw a reversed 8x16 px char without background
void PutChar_no_background_reversed (uint16_t Xpos, uint16_t Ypos, uint8_t ASCI, uint16_t charColor)
{
		uint16_t i, j;
    uint8_t buffer[16], tmp_char;
    GetASCIICode(buffer, ASCI);  /* 取字模数据 */
...
// print text without the background
...
// print reversed text without the background 
void GUI_Text_no_background_reversed (uint16_t Xpos, uint16_t Ypos, uint8_t *str,uint16_t Color)
{
		uint8_t TempChar;
    ...
    {
        TempChar = *str++;  
        PutChar_no_background_reversed(Xpos, Ypos, TempChar, Color);    
